...
Annotate.prototype = {
  init: function() {
    ...
    self.$tool += '<label class="tool-button">' +
      ...
      ' <img src="' + brush +'" /></label>' +
      '<label id="color-picker" class="tool-button">' +
      '<input type="radio" value="'+ self.options.color +'" name="' + self.toolOptionId +
      '" data-tool="null"' +
      ' data-toggle="tooltip" data-placement="top"' +
    ...
    console.log( self.$el.find('#color-picker')[0])
    self.picker = new Picker({
      parent: self.$el.find('#color-picker')[0],
      color: self.options.color,
      onChange: function(color) {
        self.options.color = color.rgbaString;
      },
    })
