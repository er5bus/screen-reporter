  ...
  switch (type) {
    case ACTIONS.ADD_INTEGRATION_SUCCEDED : {
      return { 
        ...
        integrations: state.integrations.map((item) => item.active ? {...item, active: false} : item).concat(payload) 
      }
    }
    ...
    case ACTIONS.EDIT_INTEGRATION_SUCCEDED : {
      return { ...state, success: "An integration has been set as active.", error: null, integrations: state.integrations.map((item) => item.id !== payload.id ? {...item, active: false} : {...item, ...payload}) }
    }
    case ACTIONS.EDIT_INTEGRATION_FAILED : {
      return { ...state, error: payload, success: null }
    }
    case ACTIONS.REMOVE_INTEGRATION_SUCCEDED : {
      return { ...state, error: null, success: "An integration has been removed successfully.", integrations: state.integrations.filter((item) => item.id !== payload.id )}
    }
